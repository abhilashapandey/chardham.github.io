  ...
  //Export theme options
	$( document ).on( "click", "#customize-export-custom-btn", function( e ) {
		...
		$.ajax({
			type: "post",
			url: ajaxurl,
			data: "action=wiguard-theme-options-export&nonce="+wiguard_admin_ajax_var.export_nounce,
	...
	$( document ).on( "click", "#customize-import-custom-btn", function( e ) {
		...
		if ( $( '#customize-import-value-box' ).val() === "" ) {
			...
			var post_data = { action: "wiguard-theme-option-import", nonce: wiguard_admin_ajax_var.import_nounce, json_data : json_data };
	...
	$( document ).on( "click", ".wiguard-customizer-help", function( e ) {
	...
	$( document ).on( "click", "#wiguard-customizer-clear-btn", function( e ) {
		$.ajax({
			type: "post",
			url: ajaxurl,
			data: "action=wiguard-temp-options-clear&nonce="+wiguard_admin_ajax_var.clear_nounce,
			success: function( data ){
				location.reload(true);
			}
		});
	...
	$( document ).on( "click", 'li[id^="accordion-section-wiguard_"]', function( e ) {
	...
	function customizer_set_clear_btn(stat){
		if( !$(document).find("#wiguard-customizer-clear-btn").length ){
			if( stat || customizer_fonts_vars.temp_opt_stat == '1' ){
				wp.customize.state( 'saved' ).set( false );
				var t_out = '<button title="'+ customizer_fonts_vars.temp_clear_txt +'" id="wiguard-customizer-clear-btn" class="button-primary button dashicons dashicons-admin-appearance"></button>';
	...
	function customizer_custom_refresh(stat){
		...
		if( wiguard_admin_ajax_var.customizer_load == '1' ){
	...
	function customizer_dynamic_fields_fetch( trigger_sec, field_name ){
		...
		if( $(document).find("#sub-accordion-section-"+ trigger_sec +" li.wiguard-customize-control").length ){
		...
		if( 
			...
			&&
			!$(document).find("#sub-accordion-section-"+ trigger_sec +" li.wiguard-customize-control").length
		){
			...
			$.ajax({
				type: "post",
				url: ajaxurl,
				data: "action=customizer_fields_trigger&nonce="+wiguard_admin_ajax_var.field_nounce+"&trigger_section="+trigger_sec,
				success: function( data ){
					$(document).find("#customize-control-ajax_trigger_"+ trigger_sec).remove();
					...
					//Hide description
					$(document).find("#sub-accordion-section-"+ trigger_sec).find("li.wiguard-customize-control .customize-control-description").slideUp(0);
					
					//Customizer required check process start
					customizer_required_settings();
					
					$(document).find(".wiguard-customize-control .onoffswitch-checkbox").on( "change", function() {	
						setTimeout(function(){ customizer_required_settings(); }, 500);			
					});

					$(document).find('.wiguard-customize-control select.wiguard-customizer-ajax-field').on( "change", function() {
						setTimeout(function(){ customizer_required_settings(); }, 500);
					});

					$(document).find(".wiguard-customize-control .wp-radio-image-list .wp-radio-image-field").on( "click", function() {	
						setTimeout(function(){ customizer_required_settings(); }, 500);
					});
					//Customizer required check process end
					
					//Customizer toggle tab code activate
					wiguard_customizer_toggle_section();
					...
					setTimeout(function(){ 
						$(document).find("#sub-accordion-section-"+ trigger_sec).find("li.wiguard-customize-control").addClass("control-visible");
					 }, 500);
	...
	function customizer_required_settings(){	
		var find_ele = '.wiguard-customize-control.wiguard-customize-required';
		...
		if( $(req_parent).find(find_ele).length ){
			$(req_parent).find(find_ele).each(function( index ) {
				wiguard_check_required( $(this) );
			});
		}
	}
	
	function wiguard_check_required( ele ){		

		var req_parent_id = $(ele).attr("data-required");
		var data_id = $(ele).attr("data-id");

		if( $('.wiguard-customize-control[data-id="'+ req_parent_id +'"]').attr("data-stat") == "0" ){
			$(ele).attr("data-stat", "0");
			$(ele).slideUp();
		}else{
			var req_parent = $('.wiguard-customize-control[data-id="'+ req_parent_id +'"]');
			var sel_val = wiguard_get_parent_sel_val( req_parent );
			wiguard_show_hide_customizer_fields( sel_val, ele );
		}
		
	}

	function wiguard_get_parent_sel_val( req_parent ){
		var field_type = $(req_parent).attr('data-field-type');
		var sel_val = '';
		if( field_type == 'checkbox' ){
			sel_val = $(req_parent).find(".onoffswitch-checkbox").is(":checked") ? 1 : 0;
		}else if( field_type == 'select' ){
			sel_val = $(req_parent).find('select.wiguard-customizer-ajax-field').val();
	...
	function wiguard_show_hide_customizer_fields( sel_val, field ){
	...
	function wiguard_customizer_toggle_section(){
		var find_ele = '.wiguard-customize-control.wiguard-toggle-tab-start';
		var req_parent = $(document).find('.customize-pane-child.ajax-trigger-visible.open');
		
		if( $(req_parent).find(find_ele).length ){
			$(req_parent).find(find_ele).each(function( index ) {
				$(this).nextUntil( ".wiguard-customize-control.wiguard-toggle-tab-end" ).addClass("wiguard-customize-control-close");//slideUp();
				$(this).on( "click", function(){ 
					$(this).toggleClass("opened");
					$(this).nextUntil( ".wiguard-customize-control.wiguard-toggle-tab-end" ).toggleClass("wiguard-customize-control-close");
				});
			});
		}
	}
	...
	function wiguard_open_toggle_exists(focus_ele){
		var stat = 1;
		var t_focus_ele = $(focus_ele).parent("li.wiguard-customize-control").length ? $(focus_ele).parent("li.wiguard-customize-control") : $(focus_ele).parents("li.wiguard-customize-control");
		var hard_stat = 1;
		do{
			hard_stat++;
			if( $(t_focus_ele).prev("li").length ){
				if( $(t_focus_ele).prev("li.wiguard-toggle-tab-end").length ){
					stat = 0;
				}else if( $(t_focus_ele).prev("li.wiguard-toggle-tab-start").length ){
					stat = 0;
					if( $(t_focus_ele).prev("li.wiguard-toggle-tab-start:not(.opened)").length ){
						$(t_focus_ele).prev("li.wiguard-toggle-tab-start").trigger("click");
						//toggle_stat = 1;
					}
				}
	...
	function customizer_trigger_section_focus(trigger_sec, field_name){
		...
		if( $(document).find("#sub-accordion-section-"+ trigger_sec +" li.wiguard-customize-control").length ){
			...
			var focus_ele = $(document).find("#sub-accordion-section-"+ trigger_sec +" *[name='wiguard_theme_options["+ field_name +"]'");
			wiguard_open_toggle_exists(focus_ele);
			setTimeout(function(){ 
				var t_focus_ele = $(focus_ele).parent("li.wiguard-customize-control").length ? $(focus_ele).parent("li.wiguard-customize-control") : $(focus_ele).parents("li.wiguard-customize-control");
				var li_top = $(t_focus_ele).offset();
				$( '.wp-full-overlay-sidebar-content' ).animate({ scrollTop: li_top.top }, 300);
				$(document).find('li.wiguard-customize-control.stay-focused').removeClass("stay-focused");
				$(t_focus_ele).addClass("stay-focused");
				
				$(document).find('li.wiguard-customize-control.stay-focused').on( "click", function( e ) {
	...
	function place_customizer_field_edit_buttons(iFrameDOM){
		...
		$.each( customizer_on_spot, function( index, fields ) {
			...
			customizer_onspot_txt_change( 'wiguard_theme_options['+ field_id +']', ele_selctor );
	...
	function customizer_filed_done_array_typing(cur){
		...
		formData.append( "nonce", wiguard_admin_ajax_var.field_nounce );
		...
		$.ajax({
			...
			processData: false,
			success: function( data ){
				if( $(cur).parents("li.wiguard-customize-control").attr("data-refresh") == '1' ){
	...
	function customizer_filed_done_typing(cur){
		...
		var send_data = "action=customizer_fields_custom_save&nonce="+wiguard_admin_ajax_var.field_nounce+"&field_name="+ field_name +"&field_val="+ field_val;
		$.ajax({
			type: "post",
			url: ajaxurl,
			data: send_data,
	...
	function customizer_save_values_custom_fun(){
		...
		var send_data = "action=customizer_fields_custom_save&nonce="+wiguard_admin_ajax_var.field_nounce+"&save_stat=1&form_vals="+ form_vals;
		$.ajax({
			type: "post",
			url: ajaxurl,
			data: send_data,
			success: function( data ){
				$(document).find("#wiguard-customizer-clear-btn").remove();
			}
		});
	}
	...
	function customizer_image_settings(){
		//Upload media
		$(document).find('.wiguard-customizer-image-button').on( 'click', function(e) {
			e.preventDefault();
			var cur_ele = $(this);
			var cur = $(cur_ele).parent(".wiguard-customizer-image-btn-wrap").prev(".wiguard-customizer-ajax-hid-wrap");

			//When a file is selected, grab the URL and set it as the text field's value
			custom_uploader.on('select', function() {
				
				$(this).attr("data-val", "");
				$(cur_ele).parent(".wiguard-customizer-image-btn-wrap").find("img").remove();
				...
				$(document).find('input[name="wiguard_theme_options['+ data_key +'][id]"]').val(data_id);
				$(document).find('input[name="wiguard_theme_options['+ data_key +'][url]"]').val(data_url);

				var img_out = '<img src="'+ attachment.url +'" />';				
				$(cur_ele).parent(".wiguard-customizer-image-btn-wrap").prepend(img_out);
		...
		//Remove Image
		$(document).find('.wiguard-customizer-image-remove-button').on( 'click', function(e) {
			e.preventDefault();
			var cur_ele = $(this);
			var cur = $(cur_ele).parent(".wiguard-customizer-image-btn-wrap").prev(".wiguard-customizer-ajax-hid-wrap");
			
			var data_key = $(cur).attr("data-key");
			$(document).find('input[name="wiguard_theme_options['+ data_key +'][id]"]').val('');
			$(document).find('input[name="wiguard_theme_options['+ data_key +'][url]"]').val('');
	...
	function customizer_textbox_settings(){
		$(document).find( ".wiguard-customizer-ajax-field.wiguard-customizer-text-field" ).on( "input propertychange paste", function() {
			var cur = this;
	...
	function customizer_textarea_settings(){
		$(document).find( ".wiguard-customizer-ajax-field.wiguard-customizer-textarea-field" ).on( "input propertychange paste", function() {
			var cur = this;
			customizer_on_text_typing_ajax( cur, '' );
			wp.customize.state( 'saved' ).set( false ); customizer_set_clear_btn(1);
		});
	...
	function customizer_select_settings(){
		$(document).find( ".wiguard-customizer-ajax-field.wiguard-customizer-select-field" ).on( "change", function() {
			var cur = this;
	...
	function customizer_width_height_settings(){
		$(document).find( ".wp-width-height-field" ).on( "input paste", function() {
			...
			var cur = $(cur_parent).prev(".wiguard-customizer-ajax-hid-wrap");
			var data_key = $(cur).attr("data-key");
			var data_selector = $(this).attr("data-selector");
			$(document).find('input[name="wiguard_theme_options['+ data_key +']['+ data_selector +']"]').val(cur_val);
	...
	function customizer_multi_check_settings(){
		$(document).find( ".wp-multi-check-list .wp-multi-check-field" ).on( "click", function() {
			...
			$( cur_parent ).find(".wp-multi-check-field").each(function( index ) {
				...
				var data_val = $(this).attr("data-val") ? $(this).attr("data-val") : '';
				var form_name = "wiguard_theme_options["+ data_key +"]["+ data_val +"]";
	...
	function customizer_link_color_settings(){
		...
		$(document).find(".wiguard-customize-control .link-colors-inner").on("input", function(){
	...
	function customizer_color_settings(){
		$(document).find(".wiguard-customize-color-field").wpColorPicker({
			change: function (event, ui) {
		...
		$(document).find(".wiguard-customizer-ajax-field.wiguard-customize-color-field").on("input", function(){
			customizer_on_text_typing_ajax( $(this), '' );
			wp.customize.state( 'saved' ).set( false ); customizer_set_clear_btn(1);
		});
		$(document).find(".wp-picker-clear").on("click", function(){
	...
	function customizer_alpha_color_settings(){
		$(document).find(".wiguard-customizer-alpha-color").wpColorPicker({
			change: function (event, ui) {
				var cur = event.target;
				customizer_on_text_typing_ajax( cur, '' );
				wp.customize.state( 'saved' ).set( false ); customizer_set_clear_btn(1);
		...
		$(document).find(".wiguard-customizer-ajax-field.wiguard-customizer-alpha-color").on("input", function(){
			customizer_on_text_typing_ajax( $(this), '' );
			wp.customize.state( 'saved' ).set( false ); customizer_set_clear_btn(1);
		});
		$(document).find(".wp-picker-clear").on("click", function(){
	...
	function customizer_drag_drop_settings(){
		...
		$(document).find( ".meta-drag-drop-multi-field" ).each(function() {
			...
			var final_val = $( this ).parent('.wp-drag-drop-fields').prev( ".wiguard-customizer-ajax-hid-wrap" );
			...
			$( cur_items ).each(function() {
				$( this ).sortable({
				  ...
				  update: function () {
					...
					var dd_out = '';
					var cur_parent = $( cur_items ).parents('.wp-drag-drop-fields').prev(".wiguard-customizer-ajax-hid-wrap");
					...
					$( dd_parent ).children( "ul.meta-items" ).each(function() {
						...
						$( this ).children( "li" ).each(function( index ) {
							...
							dd_out += '<input type="text" class="wiguard-customizer-ajax-field drag-drop-hid-text" name="wiguard_theme_options['+ data_key +']['+ part +']['+ data_id +']" value="'+ data_val +'">';
	...
	function customizer_background_settings(){
		...
		$(document).find('.bg-color-field').each(function(){
			...
			$(this).wpColorPicker({
				change: function(event, ui){ 
					...
					var cur = $(cur_parent).prev(".wiguard-customizer-ajax-hid-wrap");
					var data_key = $(cur).attr("data-key");
					$(document).find('input[name="wiguard_theme_options['+ data_key +'][bg_color]"]').val(color);
				...
				clear: function (event) {
					...
					var cur = $(cur_parent).prev(".wiguard-customizer-ajax-hid-wrap");
					var data_key = $(cur).attr("data-key");
					$(document).find('input[name="wiguard_theme_options['+ data_key +'][bg_color]"]').val('');
		...
		$(document).find('.bg-upload-image-button').on( 'click', function(e) {
			e.preventDefault();
			var cur_ele = $(this);
			var cur = $(cur_ele).parent(".wp-backgrounds-fields").prev(".wiguard-customizer-ajax-hid-wrap");
			...
			custom_uploader.on('select', function() {
				...
				var data_key = $(cur).attr("data-key");
				$(document).find('input[name="wiguard_theme_options['+ data_key +'][bg_media][id]"]').val(data_id);
				$(document).find('input[name="wiguard_theme_options['+ data_key +'][bg_media][url]"]').val(data_url);
		...
		$(document).find('.bg-remove-image-button').on( 'click', function(e) {
			...
			var data_key = $(cur).attr("data-key");
			$(document).find('input[name="wiguard_theme_options['+ data_key +'][bg_media][id]"]').val('');
			$(document).find('input[name="wiguard_theme_options['+ data_key +'][bg_media][url]"]').val('');
		...
		$(document).find('.wp-border-color-field').each(function(){
			$(this).wpColorPicker({
				change: function(event, ui){ 
					...
					var data_key = $(cur).attr("data-key");
					var data_selector = $(this).attr("data-selector");
					$(document).find('input[name="wiguard_theme_options['+ data_key +']['+ data_selector +']"]').val(color);
				...
				clear: function (event) {
					...
					var data_key = $(cur).attr("data-key");
					var data_selector = $(this).attr("data-selector");
					$(document).find('input[name="wiguard_theme_options['+ data_key +']['+ data_selector +']"]').val('');
