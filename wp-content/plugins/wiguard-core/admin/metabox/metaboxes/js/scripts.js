jQuery(function($) {
	...
	/*Metabox Tab*/
	$(".wiguard-admin-tabs-menu a").click(function(event) {
        event.preventDefault();
        $(this).parent().addClass("current");
        $(this).parent().siblings().removeClass("current");
        var tab = $(this).attr("href");
        $(this).parents(".wiguard-admin-tabs-menu").next(".wiguard-admin-tab").find(".wiguard-admin-tab-content").not(tab).css("display", "none");
        $(tab).fadeIn();
    });
	
	/*Wiguard Switcher*/
	$('.wiguard-switcher').change(function() {
        if($(this).is(":checked")) {
			$(this).parents('.wiguard-switch').find('.wiguard-switcher-stat').val('1');
        }else{
			$(this).parents('.wiguard-switch').find('.wiguard-switcher-stat').val('0');
		}
    });
	
	/*Wiguard Color Picker*/
	if( $(".wiguard-color-picker").length ){
		
		var patt = /^#([\da-fA-F]{2})([\da-fA-F]{2})([\da-fA-F]{2})$/;
		var color, rgb, matches; 
		$(".wiguard-color-picker").wpColorPicker({
			clear: function() { $(this).parents('td').find('.wiguard-alpha-color').val(''); },
			change: function (event, ui) {
				...
				$(this).parents('td').find('.wiguard-alpha-color').val(rgb);
				//alert(color);
				// Add your code here
			}
		});
		...
		$( ".alpha-range" ).change(function() {
			var opacity = $(this).val(); //wiguard-alpha-color-picker
			color = $(this).parents('td').find('.wiguard-alpha-color-picker').val();
			if( rgb != '' ){
				matches = patt.exec(color);
				rgb = "rgba("+parseInt(matches[1], 16)+","+parseInt(matches[2], 16)+","+parseInt(matches[3], 16)+","+ parseFloat(opacity/10) +")";
				$(this).parents('td').find('.wiguard-alpha-color').val(rgb);
	...
	jQuery(document).on( 'click', '.meta_upload_button', function( event ){
		...
		file_frame.on( 'select', function() {
			...
			jQuery.each(attachment , function(key, value){
					...
					gal_out += '<div class="zozo_gal_container" id="zozogal-'+(value.id)+'"><img src="'+(value.url)+'" alt="'+(value.id)+'" style="height:100px; width:100px;" /><span class="ti-close delgal-img" data-imgid="'+(value.id)+'"></span></div>';
			});
