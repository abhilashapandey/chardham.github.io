...
define( 'WIGUARD_METABOXES_DIR', WIGUARD_CORE_URL . 'admin/metabox/metaboxes' );
/**
 ...
 */
function wiguard_meta_box_unique_key() {
	static $meta_key = 1;
	return $meta_key++;
} 
function wiguard_post_option_get_sidebars( $name, $selected ){
	$output = '<select name="'. esc_attr( $name ) .'">';
		$output .= '<option value="">'. esc_html__( 'None', 'wiguard-core' ) .'</option>';
...
function wiguard_post_option_drag_drop($post_items, $post_items_array, $status) {
...
function wiguard_post_option_drag_drop_multi( $key, $post_items ) {
...
function wiguard_post_option_image_select( $field, $values, $default, $title) {
...
function wiguard_post_option_dd_values_check( $dd_fields, $dd_default ){
...
function wiguard_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		case 'switch':
			...
			$output .= '
				<div class="wiguard-switch">
					<label class="switch">
						<input type="checkbox" class="wiguard-switcher" '. ( $def == '1' ? 'checked' : '' ) .'>
						<div class="slider round"></div>
					</label>
					<input type="hidden" class="wiguard-switcher-stat" name="' . esc_attr( $name ) . '" value="'. esc_attr( $def ) .'" />
				</div><br />' . $desc;
		break;
		case 'dimension':
			$space_values = array();
			...
			$output .= '<div class="spacing-parent">
  					...
  					$output .= '<input size="5" type="text" name="' . esc_attr( $name .'_0') . '" value="'. esc_attr( $space_values[0] ) .'">
							...
							<select name="' . esc_attr( $name .'_1' ) . '">
								<option value="px" '. ( $space_values['1'] == 'px' ? 'selected' : '' ) .'>'. esc_html__( 'px', 'wiguard-core' ) .'</option>
								<option value="em" '. ( $space_values['1'] == 'em' ? 'selected' : '' ) .'>'. esc_html__( 'em', 'wiguard-core' ) .'</option>
								<option value="percent" '. ( $space_values['1'] == 'percent' ? 'selected' : '' ) .'>%</option>
							</select>
						</div>
						<input type="hidden" name="' . esc_attr( $name ) . '" value="wiguard_dimension" /><br />' . $desc;
		break;
		case 'space':
			$space_values = array();
			...
			$output .= '<div class="spacing-parent">
							...
							<select name="' . esc_attr( $name .'_4' ) . '">
								<option value="px" '. ( $space_values['4'] == 'px' ? 'selected' : '' ) .'>'. esc_html__( 'px', 'wiguard-core' ) .'</option>
								<option value="em" '. ( $space_values['4'] == 'em' ? 'selected' : '' ) .'>'. esc_html__( 'em', 'wiguard-core' ) .'</option>
								<option value="percent" '. ( $space_values['4'] == 'percent' ? 'selected' : '' ) .'>%</option>
							</select>';
							if( $border_style ){
								$output .= '<select name="' . esc_attr( $name .'_6' ) . '">
									<option value="solid" '. ( $space_values['6'] == 'solid' ? 'selected' : '' ) .'>'. esc_html__( 'Solid', 'wiguard-core' ) .'</option>
									<option value="dashed" '. ( $space_values['6'] == 'dashed' ? 'selected' : '' ) .'>'. esc_html__( 'Dashed', 'wiguard-core' ) .'</option>
									<option value="dotted" '. ( $space_values['6'] == 'dotted' ? 'selected' : '' ) .'>'. esc_html__( 'Dotted', 'wiguard-core' ) .'</option>
									<option value="double" '. ( $space_values['6'] == 'double' ? 'selected' : '' ) .'>'. esc_html__( 'Double', 'wiguard-core' ) .'</option>
									<option value="none" '. ( $space_values['6'] == 'none' ? 'selected' : '' ) .'>'. esc_html__( 'None', 'wiguard-core' ) .'</option>
								</select>';
							}else{
								$output .= '<input type="hidden" name="' . esc_attr( $name .'_6' ) . '" value="" />';
							}
							if( $color_opt ){
								$output .= '<input class="wiguard-color-picker" id="' . esc_attr( $id.'_5' ) . '" name="' . esc_attr( $name.'_5' ) . '" value="' . esc_attr( $space_values[5] ) . '" type="text" size="10" />';
							}else{
								$output .= '<input type="hidden" name="' . esc_attr( $name .'_5' ) . '" value="" />';
							}
			$output .= '</div>
						<input type="hidden" name="' . esc_attr( $name ) . '" value="wiguard_space" /><br />' . $desc;
		break;
		...
		case 'link_color':
			...
			$output .= '<div class="link-color-parent">
							<i>'. esc_html__( 'Regular', 'wiguard-core' ) .'</i><input type="text" name="' . esc_attr( $name.'_0' ) . '" class="wiguard-color-picker" id="' . esc_attr( $id.'_0' ) . '" value="' . esc_attr( $link_values[0] ) . '" size="10"  />
							<i>'. esc_html__( 'Hover', 'wiguard-core' ) .'</i><input type="text" name="' . esc_attr( $name.'_1' ) . '" class="wiguard-color-picker" id="' . esc_attr( $id.'_1' ) . '" value="' . esc_attr( $link_values[1] ) . '" size="10"  />
							<i>'. esc_html__( 'Active', 'wiguard-core' ) .'</i><input type="text" name="' . esc_attr( $name.'_2' ) . '" class="wiguard-color-picker" id="' . esc_attr( $id.'_2' ) . '" value="' . esc_attr( $link_values[2] ) . '" size="10"  />
						</div>
						<input type="hidden" name="' . esc_attr( $name ) . '" value="wiguard_link_color" /><br />' . $desc;
		break;
		...
		case 'alpha_color':
			...
			$output .= '<div class="wiguard-alpha-color">';
				$output .= '<input type="text" name="' . esc_attr( $name.'_0' ) . '" id="' . esc_attr( $id.'_0' ) . '" value="' . esc_attr( $alpha_values[0] ) . '" class="wiguard-alpha-color" />
						<input class="wiguard-color-picker wiguard-alpha-color-picker" id="' . esc_attr( $id.'_0' ) . '" name="' . esc_attr( $name.'_1' ) . '" value="' . esc_attr( $alpha_values[1] ) . '" type="text" size="10" />
						<input type="range" min="1" max="10" step="1" value="3" class="alpha-range">';
				$output .= '<input type="hidden" name="' . esc_attr( $name ) . '" value="wiguard_alpha_color" /><br />' . $desc;
			$output .= '</div>';
		break;
		...
		break;
		case 'rating':
			$output .= '<ul class="wiguard-meta-rating star-rating">
						...
						<input type="hidden" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . absint( $meta ) . '" class="wiguard-meta-rating-value" />
					<br />' . $desc;
		break;
		case 'number':
		...
		// color
		case 'color':
			$output .= '<input type="text" name="' . esc_attr( $name ) . '" class="wiguard-color-picker" id="' . esc_attr( $id ) . '" value="' . esc_attr( $meta ) . '" size="10"  />';
		break;
		...
		// image
		case 'image':
			$image = WIGUARD_METABOXES_DIR . '/images/image.png';	
		...
		case 'repeatable':
			...
			foreach( $meta as $row ) {
				...
				foreach ( $repeatable_fields as $repeatable_field ) {
					...
					$output .= wiguard_meta_box_field( $repeatable_field, $meta[$i][$repeatable_field['id']], array( $id, $i ) );
					$output .= '</p>';
				} // end each field
							...
							<div class="zozo_gallery_images">';
								if( $meta ){
									...
									foreach( $images as $image ) {
										$output .= '<div class="zozo_gal_container" id="zozogal-'. esc_attr( $image ) .'"><img src="'. wp_get_attachment_url($image) .'" alt="'.  esc_html( $image ) .'" style="height:100px; width:100px;" /><span class="ti-close delgal-img" data-imgid="'. esc_attr( $image ) .'"></span></div>';
									}
								}
		...
		case 'dragdrop':
			...
			$t_meta_key = wiguard_meta_box_unique_key();
			$meta = isset( $meta ) && $meta != '' ? $meta : $default;
			...
			$post_enabled_out = $post_enabled_items != '' ? wiguard_post_option_drag_drop( $post_enabled_items, $options['items'], 'enabled' ) : '';
			$post_disabled_out =  $post_disabled_items != '' ? wiguard_post_option_drag_drop( $post_disabled_items, $options['items'], 'disabled' ) : '';
			
			$output .= '<div class="meta-drag-drop-field" data-field="'. $t_meta_key .'">';
				$output .= '<h4>'. esc_html__( 'Enabled Items', 'wiguard-core' ) .'</h4>';
				$output .= $post_enabled_out;
				$output .= '<h4>'. esc_html__( 'Disabled Items', 'wiguard-core' ) .'</h4>';
		...
		case 'dragdrop_multi':
			...
			//$meta = '';
			if( isset( $meta ) && $meta != '' ){
				$saved_dd = json_decode( stripslashes( $meta ), true );
			}else{
				$saved_dd = $dd_fields;
			}
			$part_array = wiguard_post_option_dd_values_check( $saved_dd, $dd_fields );
			
			$t_part_array = array();
			$f_part_array = array();
			foreach( $part_array as $key => $value ){
				$t_part_array[$key] = $value != '' ? wiguard_post_option_drag_drop_multi( $key, $value ) : '';
			}
			...
			$output .= '<input id="' . esc_attr( $id ) . '" class="meta-drag-drop-multi-value" name="' . esc_attr( $name ) . '" value="" data-params="'. htmlspecialchars( json_encode( $part_array ), ENT_QUOTES, 'UTF-8' ) .'" type="hidden">';
			$output .= '</div>'. $desc;
		break;
		...
		case 'image_select':
			...
			$output .= wiguard_post_option_image_select( $name, $options, $selected, $label );
		...
		case 'sidebar':
			...
			$selected = isset( $meta ) && $meta != '' ? $meta : $default;
			$output .= wiguard_post_option_get_sidebars( $name, $selected);
...
class Custom_Add_Meta_Box {
	...
	function admin_enqueue_scripts() {
		...
		if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
			...
			if ( meta_box_find_field_type( 'alpha_color', $this->fields ) ){
				wp_enqueue_style( 'rangeslider', WIGUARD_METABOXES_DIR . '/css/rangeslider.css', array(), '1.0' );
				wp_enqueue_script( 'jquery-rangeslider', WIGUARD_METABOXES_DIR . '/js/rangeslider.min.js', array( 'jquery' ), '1.0', true );
			}
			
			wp_enqueue_script( 'meta_box', WIGUARD_METABOXES_DIR . '/js/scripts.js', $deps );
			
			// css
			$deps = array();
			wp_register_style( 'jqueryui', WIGUARD_METABOXES_DIR . '/css/jqueryui.css' );
			if ( meta_box_find_field_type( 'date', $this->fields ) || meta_box_find_field_type( 'slider', $this->fields ) )
				$deps[] = 'jqueryui';
			if ( meta_box_find_field_type( 'color', $this->fields ) )
				$deps[] = 'farbtastic';
			wp_enqueue_style( 'meta_box', WIGUARD_METABOXES_DIR . '/css/meta_box.css', $deps );
		}
	}
	...
	function meta_box_callback() {
		...
		wp_nonce_field( 'wiguard_meta_box_nonce_action', 'wiguard_meta_box_nonce_field' );
		...
		foreach ( $this->fields as $field) {
			...
			if( isset( $field['required'] ) )
				...
				if( $field['type'] != 'label' && $field['type'] != 'line' ){
					...
					$meta_output[$tab] .= wiguard_meta_box_field( $field, $meta  );
		...
		foreach($meta_output as $key => $value){
			...
			$tab_list .= '<li class="'. ( $i == 0 ? 'current' : '' ) .'"><a href="#wiguard-admin-tab-'. esc_attr( $tab_id ) .'">'. esc_html( $key ) .'</a></li>';
			$tab_content .= '<div id="wiguard-admin-tab-'. esc_attr( $tab_id ) .'" class="wiguard-admin-tab-content'. ( $i == 0 ? ' current' : '' ) .'">';
		...
		echo '
		<div id="wiguard-admin-tabs-container">
			<ul class="wiguard-admin-tabs-menu">
				'. $tab_list .'
			</ul>
			<div class="wiguard-admin-tab">
				'. $tab_content .'
			</div>
		</div>';
	}
	...
	function save_box( $post_id ) {
		...
		// verify nonce
		if ( ! isset( $_POST['wiguard_meta_box_nonce_field'] ) )
			return $post_id;
		if ( ! ( in_array( $post_type, $this->page ) || wp_verify_nonce( $_POST['wiguard_meta_box_nonce_field'],  'wiguard_meta_box_nonce_action' ) ) ) 
			return $post_id;
		// check autosave
