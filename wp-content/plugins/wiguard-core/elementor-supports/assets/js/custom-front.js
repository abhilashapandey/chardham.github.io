( function( $ ) {
	...
	var WidgetAnimateTextHandler = function( $scope, $ ) {
		$scope.find('.typing-text').each(function( index ) {
			wiguard_animated_text_settings( this, index );
		});
	};
	...
	var WidgetCircleProgressHandler = function( $scope, $ ) {
		if( $scope.find('.circle-progress-circle').length ){
			...
			wiguard_circle_progres_settings(circle_ele);
		}		
	};
	...
	var WidgetCounterUpHandler = function( $scope, $ ) {
		if( $scope.find('.counter-up').length ){
			...
			wiguard_counter_up_settings(counter_ele);
		}		
	};
	...
	var WidgetDayCounterHandler = function( $scope, $ ) {
		$scope.find('.day-counter').each(function() {
			wiguard_day_counter_settings( this );
		});		
	};
	...
	var WidgetChartHandler = function( $scope, $ ) {
		$scope.find('.pie-chart').each(function() {
			wiguard_pie_chart_settings( this );
		});		
		$scope.find('.line-chart').each(function() {
			wiguard_line_chart_settings( this );
		});		
	};
	...
	var WidgetRoundTabHandler = function( $scope, $ ) {
		$scope.find('.round-tab-wrapper').each(function() {
			wiguard_round_tab_settings( this );
		});		
	};
	...
	var WidgetRecentPopularToggleHandler = function( $scope, $ ) {
		if( $scope.find(".wiguard-toggle-post-trigger").length ){
			$scope.find(".wiguard-toggle-post-trigger .switch-checkbox").change(function(){
				wiguard_switch_tab_toggle( this );
			});
		}
	};
	...
	var SectionCustomOptionsHandler = function( $scope, $ ) {
		...
		if ( $scope.is('section' ) ){
			if ( $scope.is('section[data-wiguard-float]' ) ){
				wiguard_section_float_parallax( $scope );
			}
			if ( $scope.is('section[data-wiguard-raindrops]' ) ){
				wiguard_section_rain_drops( $scope );
			}
			if ( $scope.is('section[data-wiguard-parallax-data]' ) ){
				wiguard_section_parallax( $scope );
			}
		}
	};
	...
	var ColumnCustomOptionsHandler = function( $scope, $ ) {
		if ( $scope.is('.elementor-element.elementor-column' ) ){
			if ( $scope.is('.elementor-element.elementor-column[data-wiguard-slide]' ) ){
				wiguard_content_slider( $scope );
			}
		}
	};
	...
	$( window ).on( 'elementor/frontend/init', function() {
		...
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardcircleprogress.default', WidgetCircleProgressHandler );
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardcounter.default', WidgetCounterUpHandler );
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguarddaycounter.default', WidgetDayCounterHandler );
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardimagegrid.default', WidgetOwlCarouselHandler );
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardmodalpopup.default', WidgetModalPopupHandler );
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardtimeline.default', WidgetAgonHandler );
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardtimelineslide.default', WidgetTimelineSliderHandler );
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardchart.default', WidgetChartHandler );
		...
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardblog.default', WidgetIsotopeHandler );
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardblog.default', WidgetOwlCarouselHandler );
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardrecentpopular.default', WidgetRecentPopularToggleHandler );
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardteam.default', WidgetOwlCarouselHandler );
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardevent.default', WidgetOwlCarouselHandler );
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardservice.default', WidgetOwlCarouselHandler );
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardtestimonial.default', WidgetOwlCarouselHandler );
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardportfolio.default', WidgetIsotopeHandler );
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardportfolio.default', WidgetOwlCarouselHandler );
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardportfolio.default', WidgetZoomGalleryHandler );
		
		// For a widget without a skin (skin = default)
		elementorFrontend.hooks.addAction( 'frontend/element_ready/wiguardproducts.default', function( $scope ) {
			console.log("wiguard products ready");
	...
	function wiguard_scroll_animation( c_elem ){
		setTimeout( function() {
			var anim_time = 300;
			$(c_elem).find('.wiguard-animate:not(.run-animate)').each( function() {
	...
	function wiguardzozotimeline( cur_ele ){
		var cur_ele = $(cur_ele);
	...
	function wiguard_toggle_content( toggle_ele ){
	...
	function wiguard_popup_gallery( popup_ele ){
	...
	function wiguard_round_tab_settings( tab_ele ){
		var tab_ele = $(tab_ele);
	...
	function wiguard_section_rain_drops( rd_ele ){
		rd_ele.addClass("section-raindrops-actived");
		var rd_json = JSON.parse(decodeURIComponent(rd_ele.attr("data-wiguard-raindrops")));
		rd_ele.append('<div class="wiguard-raindrops-wrap"></div>');
		...
		if( rd_pos == "top" ){
			rd_ele.find(".wiguard-raindrops-wrap").css({"top" : "-"+ rd_height +"px"});
		}else{
			rd_ele.find(".wiguard-raindrops-wrap").css({"bottom" : "0"});
		}
		
		rd_ele.find(".wiguard-raindrops-wrap").css("height", rd_height + "px");
		
		var rain_ele = rd_ele.find(".wiguard-raindrops-wrap").raindrops({
			color: rd_color,
	...
	function wiguard_content_slider( slide_ele ){
		var slide_ele = $(slide_ele);
		var slide_json = JSON.parse(decodeURIComponent(slide_ele.attr("data-wiguard-slide")));
		...
		wiguard_owl_json_settings(children_ele, slide_json);
	}
	
	function wiguard_section_parallax( pr_ele ){
		
		var pr_ele = $(pr_ele);
		var pr_json = JSON.parse(decodeURIComponent(pr_ele.attr("data-wiguard-parallax-data")));
		
		var parallax_ratio = pr_json.parallax_ratio;
		var parallax_img = pr_json.parallax_image;

		pr_ele.prepend('<div class="wiguard-parallax" data-wiguard-parallax data-speed="'+ parallax_ratio +'" style="background-image:url('+ parallax_img +')"></div>');
		...
		$(pr_ele).find('.wiguard-parallax').each(function(){
	...
	function wiguard_section_float_parallax( pr_ele ){
		
		var pr_ele = $(pr_ele);
		var pr_json = JSON.parse(decodeURIComponent(pr_ele.attr("data-wiguard-float")));
		var fload_id = 1;
		...
		$.each( pr_json, function(idx, obj) {
			...
			$("#float-parallax-"+fload_id).wiguardparallax({
				t_top: float_top,
				t_left: float_left,
	...
	function wiguard_switch_tab_toggle( toggle_ele ){
		if( toggle_ele.checked ) {
			var toggle_ele = $(toggle_ele);
            toggle_ele.parents(".wiguard-toggle-post-wrap").addClass("wiguard-active-post");
        }else{
			var toggle_ele = $(toggle_ele);
			toggle_ele.parents(".wiguard-toggle-post-wrap").removeClass("wiguard-active-post");
		}
	}
	...
	function wiguard_counter_up_settings( counterup ){
	...
	function wiguard_day_counter_settings( day_counter ){
	...
	function wiguard_pie_chart_settings( chart_ele ){
	...
	function wiguard_line_chart_settings( chart_ele ){
		var chart_ele = $( chart_ele );
		var c_chart = $('#' + chart_ele.attr("id") );
		var chart_labels = c_chart.attr("data-labels");
		var chart_values = c_chart.attr("data-values");
	...
	function wiguard_animated_text_settings( cur_ele, index ){
		var cur_ele = $(cur_ele);
	...
	function wiguard_circle_progres_settings( circle_ele ){
	...
	function wiguard_isotope_layout( c_elem ){
		var c_elem = $(c_elem);
		...
		$(c_elem).prev(".isotope-filter").find( ".isotope-filter-item" ).on( 'click', function() {
			...
			if( c_elem.find( ".isotope-item" + filter ).hasClass( "wiguard-animate" ) ){
				if( filter ){
					c_elem.find( ".isotope-item" + filter ).removeClass("run-animate");
				}else{
					c_elem.find( ".isotope-item" ).removeClass("run-animate");
				}
				wiguard_scroll_animation(c_elem);
			}
			cur_isotope.isotope({ 
				filter: filter
			});
		...
		if( c_elem.find( ".isotope-item" ).hasClass( "wiguard-animate" ) ){
			wiguard_scroll_animation(c_elem);
	...
	function wiguard_owl_json_settings(c_owlCarousel, data_json){
		var c_owlCarousel = $(c_owlCarousel);
		// Data Properties
