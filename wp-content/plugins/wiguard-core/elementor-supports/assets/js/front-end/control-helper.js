	...
	jQuery( window ).on( 'elementor:init', function() {
		...
		var ControlItemSpacingItemView = elementor.modules.controls.BaseData.extend( {
			onReady: function() {
				...
				var inp = this.ui.textarea;
				if( $("#elementor-preview-iframe").contents().find( ".wiguard-inline-css" ).length ){
					/* Shortcode CSS Append */
					var css_out = '';
					$("#elementor-preview-iframe").contents().find( ".wiguard-inline-css" ).each(function() {
						var shortcode = $( this );
						var shortcode_css = shortcode.attr("data-css");		
						css_out += ($).parseJSON( shortcode_css );
					});
					if( css_out != '' ){
						$("#elementor-preview-iframe").contents().find( "head #wiguard-customizer-css" ).remove();
						$("#elementor-preview-iframe").contents().find( "head" ).append( '<style id="wiguard-customizer-css">'+ css_out +'</style>' );
					}
				}
