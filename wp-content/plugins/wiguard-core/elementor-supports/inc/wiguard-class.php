...
class CEAPostElements {
	...
	function wiguard_wp_bootstrap_pagination( $args = array(), $max = '', $print = true ) {
		...
		$defaults = array(
			'range'           => 4,
			'custom_query'    => false,
			'first_string' => esc_html__( 'First', 'wiguard-core' ),
			'previous_string' => esc_html__( 'Prev', 'wiguard-core' ),
			'next_string'     => esc_html__( 'Next', 'wiguard-core' ),
			'last_string'     => esc_html__( 'Last', 'wiguard-core' ),
		...
		if ( $firstpage && (1 != $page) && isset( $args['first_string'] ) && $args['first_string'] != '' )
			$echo .= '<li class="nav-item previous"><a href="' . $firstpage . '" title="' . esc_attr__( 'First', 'wiguard-core') . '">' . $args['first_string'] . '</a></li>';
		if ( $previous && (1 != $page) )
			$echo .= '<li class="nav-item"><a href="' . $previous . '" title="' . esc_attr__( 'previous', 'wiguard-core') . '">' . $args['previous_string'] . '</a></li>';
		...
		if ($next && ($count != $page) )
			$echo .= '<li class="nav-item"><a href="' . $next . '" class="next-page" title="' . esc_attr__( 'next', 'wiguard-core') . '">' . $args['next_string'] . '</a></li>';
		
		$lastpage = esc_attr( get_pagenum_link($count) );
		if ( $lastpage && isset( $args['last_string'] ) && $args['last_string'] != '' ) {
			$echo .= '<li class="nav-item next"><a href="' . $lastpage . '" title="' . esc_attr__( 'Last', 'wiguard-core') . '">' . $args['last_string'] . '</a></li>';
		}
		if ( isset($echo) && $print ){
	...
	function wiguard_breadcrumbs() {
	  ...
	  $home = esc_html__('Home', 'wiguard-core'); // text for the 'Home' link
	  ...
	  if (is_home() || is_front_page()) {  
		...
		if ( !is_single() && !is_page() && get_post_type() != 'post' && !is_404() ) {
		  ...
		  echo wp_kses_post( $before . esc_html__('Posts by ', 'wiguard-core') . $userdata->display_name . $after );
	 
		} elseif ( is_404() ) {
		  echo wp_kses_post( $before . esc_html__('Error 404', 'wiguard-core') . $after );
		}
	 
		if ( get_query_var('paged') ) {
		  if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ' (';
		  echo esc_html__('Page', 'wiguard-core') . ' ' . get_query_var('paged');
		  if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
		}
	  }
	  echo '</div>';
