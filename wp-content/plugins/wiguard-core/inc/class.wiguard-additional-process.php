...
class wiguard_additional_process {
	
	public static function wiguard_favourite_ip_verify( $postID ){
		// Retrieve post votes IPs
		$meta_IP = get_post_meta( $postID, 'wiguard_favourite_IP', true );
		if( isset( $meta_IP ) && is_array( $meta_IP ) ){
			// Retrieve current user IP
			$ip = class_exists( "WiguardFamework" ) ? wiguard_get_remote_ip() : '1';
	...
	public static function wiguard_meta_favourite($postID){
		$output = '';
		$meta_IP = get_post_meta( $postID, 'wiguard_favourite_IP', true );
		$ip = class_exists( "WiguardFamework" ) ? wiguard_get_remote_ip() : '1';
		
		$meta_count = get_post_meta( $postID, 'wiguard_post_fav_count', true );
		$meta_count = $meta_count != '' ? $meta_count : '0';
		
		$like_text = esc_html__( 'Likes', 'wiguard' );
		if( $meta_count <= 1 ){
			$like_text = esc_html__( 'Like', 'wiguard' );
		}
		
		$output .= '<ul class="nav post-fav-wrap">';
		if( self::wiguard_favourite_ip_verify( $postID ) ){
	...
	public static function wiguard_meta_favourite_check(){
		// Check for nonce security
		$nonce = $_POST['nonce'];  
		if ( ! wp_verify_nonce( $nonce, 'wiguard-post-fav' ) )
		...
		if( $postID != '' )
		{
			...
			$meta_key = '';
			$meta_count = 0;
			$meta_key = 'wiguard_post_fav_count';
			...
			if( ! self::wiguard_favourite_ip_verify( $postID ) )	{
				...
				update_post_meta( $postID, "wiguard_favourite_IP", $meta_IP );
			...
			echo self::wiguard_meta_favourite( $postID );
		}
		exit;
	}
	
	public static function wiguard_comment_id_verify( $comment_id ){
		// Retrieve post votes IPs
	...
	public static function wiguard_comments_like(){
		// Check for nonce security
		$nonce = $_POST['nonce'];  
		if ( ! wp_verify_nonce( $nonce, 'wiguard-comment-like' ) )
			die ( esc_html__( 'Busted', 'wiguard' ) );
		...
		if(isset($_POST['cmt_id']))
		{
			...
			// Use has already voted ?
			if( ! self::wiguard_comment_id_verify( $comment_id ) )
			{
				if( isset( $meta_IP ) && is_array( $meta_IP ) ){
					if( $comment_meta == '1' ){
						$meta_IP[$ip] = 'like';
				...
				echo self::wiguard_comment_like_out( $comment_id );
			}else{
	...
	public static function wiguard_comment_like_out( $comment_id ){
		$output = '';
		$meta_IP = get_comment_meta( $comment_id, 'comment_voted_IP', true );
		//print_r($meta_IP);
		$ip = class_exists( "WiguardFamework" ) ? wiguard_get_remote_ip() : '1';
	...
	public static function wiguard_comment_share( $comment_id ){
		$output = '';
		$comments_shares = WiguardFamework::wiguard_static_theme_mod( 'comments-social-shares' );
		ob_start();
	?>
...
add_action( 'wp_ajax_post_fav_act', array( 'wiguard_additional_process', 'wiguard_meta_favourite_check' ) );
add_action( 'wp_ajax_nopriv_post_fav_act', array( 'wiguard_additional_process', 'wiguard_meta_favourite_check' ) );

if( WiguardFamework::wiguard_static_theme_mod( 'comments-like' ) ){
	add_action('wp_ajax_nopriv_comment_like', array( 'wiguard_additional_process', 'wiguard_comments_like' ) );
	add_action('wp_ajax_comment_like', array( 'wiguard_additional_process', 'wiguard_comments_like' ) );
}
