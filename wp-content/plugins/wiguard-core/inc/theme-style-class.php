...
class WiguardThemeStyles {
	...
	public function __construct(){
		self::$wiguard_style_mod = get_option( 'wiguard_theme_options_new');
	}
		
	public static function wiguard_static_theme_mod($field){
		if( is_customize_preview() ){
			$wiguard_mod_t = get_option( 'wiguard_theme_options_t');
			$wiguard_style_mod = !empty( $wiguard_mod_t ) ? $wiguard_mod_t : self::$wiguard_style_mod;
		}else{
			$wiguard_style_mod = self::$wiguard_style_mod;
		}
		
		return isset( $wiguard_style_mod[$field] ) && $wiguard_style_mod[$field] != '' ? $wiguard_style_mod[$field] : '';
	}
	
	function wiguard_theme_color(){
		$theme_color = self::wiguard_static_theme_mod( 'theme-color' );
		return $theme_color ? $theme_color : '#54a5f8';
	}
	function wiguard_secondary_color(){
		$sec_color = self::wiguard_static_theme_mod( 'secondary-color' );
		return $sec_color ? $sec_color : '#95ce69';
	}

	function wiguard_container_width(){
		$site_width = self::wiguard_static_theme_mod( 'site-width' );
		return !empty( $site_width ) ? absint( $site_width ) . 'px' : '1140px';
	}
	
	function wiguard_border_settings($field){
		$border = self::wiguard_static_theme_mod( $field );
		if( !empty( $border ) ):
			...
			if( isset( $border['top'] ) && $border['top'] != '' ):
				echo '
				border-top-width: '. esc_attr( $border['top'] ) .'px;
				border-top-style: '. esc_attr( $boder_style ) .';
				border-top-color: '. esc_attr( $border_color ) .';';
			endif;
			
			if( isset( $border['right'] ) && $border['right'] != '' ):
				echo '
				border-right-width: '. esc_attr( $border['right'] ) .'px;
				border-right-style: '. esc_attr( $boder_style ) .';
				border-right-color: '. esc_attr( $border_color ) .';';
			endif;
			
			if( isset( $border['bottom'] ) && $border['bottom'] != '' ):
				echo '
				border-bottom-width: '. esc_attr( $border['bottom'] ) .'px;
				border-bottom-style: '. esc_attr( $boder_style ) .';
				border-bottom-color: '. esc_attr( $border_color ) .';';
			endif;
			
			if( isset( $border['left'] ) && $border['left'] != '' ):
				echo '
				border-left-width: '. esc_attr( $border['left'] ) .'px;
				border-left-style: '. esc_attr( $boder_style ) .';
	...
	function wiguard_padding_settings($field){
		$padding = self::wiguard_static_theme_mod( $field );
		if( !empty( $padding ) ):
			echo isset( $padding['top'] ) && $padding['top'] != '' ? 'padding-top: '. esc_attr( $padding['top'] ) .'px;' : '';
			echo isset( $padding['right'] ) && $padding['right'] != '' ? 'padding-right: '. esc_attr( $padding['right'] ) .'px;' : '';
			echo isset( $padding['bottom'] ) && $padding['bottom'] != '' ? 'padding-bottom: '. esc_attr( $padding['bottom'] ) .'px;' : '';
			echo isset( $padding['left'] ) && $padding['left'] != '' ? 'padding-left: '. esc_attr( $padding['left'] ) .'px;' : '';
		endif;
	}
	
	function wiguard_margin_settings( $field ){
		$margin = self::wiguard_static_theme_mod( $field );
		if( $margin ):
	...
	function wiguard_link_color($field, $fun, $echo = true){
		$link = self::wiguard_static_theme_mod( $field );
		if( $echo ){
	...
	function wiguard_bg_rgba($field){
		$bg_rgba = self::wiguard_static_theme_mod( $field );
		if( $bg_rgba != '' ) echo 'background: '. esc_attr( $bg_rgba ) .';';
	}
	
	function wiguard_bg_settings($field){
		$bg = self::wiguard_static_theme_mod( $field );
		if( !empty( $bg ) ):
	...
	function wiguard_custom_font_face_create( $font_family, $cf_names ){
	...
	function wiguard_custom_font_check($field){
		$cf = self::wiguard_static_theme_mod( $field );
		...
		if ( !empty( $cf_names ) && !in_array( $f_family, $this->exists_fonts ) ){
			$this->wiguard_custom_font_face_create( $f_family, $cf_names );
			array_push( $this->exists_fonts, $f_family );
		}
	}
	
	function wiguard_get_custom_google_font_frame( $font_family ){
	...
	function wiguard_typo_generate($field){
		$typo = self::wiguard_static_theme_mod( $field );
		if( $typo ):
			...
			if ( !in_array( $font_family, $cf_names ) && !in_array( $font_family, $this->_standard_fonts ) && !empty( $font_family ) ){
				...
				$this->wiguard_get_custom_google_font_frame( $gf_arr );
			}
			
		endif;
	}
	
	function wiguard_typo_ouput($field){
		$typo = self::wiguard_static_theme_mod( $field );
		if( $typo ):
	...
	function wiguard_hex2rgba($color, $opacity = 1) {
