...
class Zozo_Tweets_Widget extends WP_Widget {
	...
	public function __construct() {
		/* Widget settings. */
		$widget_options = array('classname' => 'zozo_tweets_widget', 'description' => esc_html__( 'Displays Twitter feeds.', 'wiguard-core' ) );
		$control_options = array('id_base' => 'zozo_tweets-widget');
		
		/* Create the widget. */
		parent::__construct('zozo_tweets-widget', esc_html__( 'Wiguard Twitter Feeds', 'wiguard-core' ), $widget_options, $control_options);
	}
	function widget($args, $instance)
	{
		...
		$tweets = wiguard_get_tweets( $consumer_key, $consumer_secret, $access_token, $access_token_secret, $tweet_count, $twitter_id );
				...
				<ul class="twitter-items">
					<?php foreach($tweets as $tweet) { ?>
						<li class="tweet-item">
							...
							$time_ago = wiguard_tweet_time_ago($tweet_time); ?>
...
function wiguard_get_tweets( $consumer_key, $consumer_secret, $access_token, $access_token_secret, $tweet_count, $twitter_id ){
	// Include Main Library File
	require_once( WIGUARD_CORE_DIR . 'widgets/twitter/twitteroauth.php' );
	...
	if( ( false === $cached_twitter_feeds || empty( $cached_twitter_feeds ) ) || $tweet_count > count( $cached_twitter_feeds ) ) {
		...
		// Get Access Token
		$connection = wiguard_get_connection_with_access_token($consumer_key, $consumer_secret, $access_token, $access_token_secret);				
		$params = array(
		  'count' 		=> $tweet_count,
...
function wiguard_tweet_time_ago( $time ) {
...
function wiguard_get_connection_with_access_token($cons_key, $cons_secret, $oauth_token, $oauth_token_secret) 
{
