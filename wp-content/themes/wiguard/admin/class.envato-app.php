...
class Zozo_Envato_API {
	...
	public function request( $url ) {
		...
		if ( empty( $this->token ) ) {
			return new WP_Error( 'api_token_error', esc_html__( 'An API token is required.', 'wiguard' ) );
		}
		...
		if ( 200 !== $response_code ) {
			return new WP_Error( $response_code, esc_html__( 'An unknown API error occurred.', 'wiguard' ) );
		}
		$body_data = json_decode( wp_remote_retrieve_body( $response ), true );
		
		if ( null === $body_data ) {
			return new WP_Error( 'api_error', esc_html__( 'An unknown API error occurred.', 'wiguard' ) );
		}
	...
	public function check_is_theme_purchase(){
		...
		if ( empty( $this->product_id ) ) {
			return new WP_Error( 'product_id_error', esc_html__( 'Product ID is required.', 'wiguard' ) );
		}
