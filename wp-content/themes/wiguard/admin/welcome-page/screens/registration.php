...
<div class="wrap about-wrap welcome-wrap zozothemes-wrap">
	...
	<div class="zozothemes-welcome-inner">
		...
		<h2 class="zozo-nav-tab-wrapper nav-tab-wrapper">
			...
			printf( '<a href="#" class="nav-tab nav-tab-active">%s</a>', esc_html__( "Registration", "wiguard" ) );
	...
	?>
		...
		<div class="zozo-envato-registration-form-wrap">
			<p><?php esc_html_e( "Please enter your Envato token to complete registration.", "wiguard" ); ?></p>
			<form id="zozo-envato-registration-form" method="post">
				<div class="form-fields">
					...
					<input type="submit" name="submit" id="submit" class="button button-primary button-large" value="<?php echo esc_html_e( "Submit", "wiguard" ); ?>">
				</div>
				<?php if( !empty( $verfied_stat ) && $verfied_stat == 'invalid' ): ?>
					<span class="invalid-token-txt"><?php esc_html_e( "Invalid token id. Please enter valid token id. Refer below instructions.", "wiguard" ); ?></span>
				<?php endif; ?>
			</form>
			...
			<div class="registration-token-instruction">
				<ol class="list">
					...
					esc_html__( 'Click on this', 'wiguard' ),
					esc_url( 'https://build.envato.com/create-token/?user:username=t&amp;purchase:download=t&amp;purchase:verify=t&amp;purchase:list=t' ),
					esc_html__( 'Generate A Personal Token', 'wiguard' ),
					esc_html__( 'link.', 'wiguard' ),
					esc_html__( 'IMPORTANT:', 'wiguard' ),
					esc_html__( 'You must be logged into the same Themeforest account that purchased', 'wiguard' ),
					esc_html( $zozo_theme_name ),
					esc_html__( 'If you are logged in already, look in the top menu bar to ensure it is the right account. If you are not logged in, you will be directed to login then directed back to the Create A Token Page.', 'wiguard' )
					); ?></li>
					<li><?php 
						printf( '%1$s <strong>%2$s</strong> %3$s <strong>%4$s</strong> %5$s <strong>%6$s</strong>',
						esc_html__( 'Enter a name for your token, then check the boxes for', 'wiguard' ),
						esc_html__( 'View Your Envato Account Username, Download Your Purchased Items, List Purchases You\'ve Made', 'wiguard' ),
						esc_html__( 'and', 'wiguard' ),
						esc_html__( 'Verify Purchases You\'ve Made', 'wiguard' ),
						esc_html__( 'from the permissions needed section. Check the box to agree to the terms and conditions, then click the', 'wiguard' ),
						esc_html__( 'Create Token button', 'wiguard' )
						);
					?></li>
					<li><?php esc_html_e( 'A new page will load with a token number in a box. Copy the token number then come back to this registration page and paste it into the field below and click the Submit button.', 'wiguard' ); ?></li>
					<li><?php esc_html_e( 'You will see a green check mark for success, or a failure message if something went wrong. If it failed, please make sure you followed the steps above correctly.', 'wiguard' ); ?></li>
				</ol>
			</div>
		</div>
