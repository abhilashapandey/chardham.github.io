 ...
 */
class wiguard_wp_bootstrap_navwalker extends Walker_Nav_Menu {
	private $megamenu = '';
	private $megabgimg = '';
	private $megafull = '';
	private $submegamenu = '';
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		if ( class_exists( 'Wiguard_Walker_Nav_Menu_Edit_Custom' ) ) {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( class_exists( 'Wiguard_Walker_Nav_Menu_Edit_Custom' ) ) {
			...
			if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
				...
				$hot_icon = '';
				if( $item->popbtn != '' && WiguardThemeOpt::wiguard_static_theme_mod('menu-tag') == 1 ){
					$tag_text = '';
					if( WiguardThemeOpt::wiguard_static_theme_mod('menu-tag-'.$item->popbtn.'-text') != '' )
						$tag_text = esc_html( WiguardThemeOpt::wiguard_static_theme_mod('menu-tag-'.$item->popbtn.'-text') );
				...
				if( $depth == 0 && $item->megamenulogo == 'on' ){
					...
					$item_output .= $ahe->wiguard_header_logo();
					$item_output .= '<div class="sticky-logo">'. $ahe->wiguard_additional_logo('sticky-logo') .'</div>';
	...
	public static function fallback( $args ) {
		if ( current_user_can( 'manage_options' ) ) {
			...
			$fb_output .= '>';
			$fb_output .= '<li><a href="' . admin_url( 'nav-menus.php' ) . '">'. esc_html__('Add a menu', 'wiguard') .'</a></li>';
			$fb_output .= '</ul>';
			if ( $container )
				$fb_output .= '</' . $container . '>';
			echo ( ''. $fb_output );
