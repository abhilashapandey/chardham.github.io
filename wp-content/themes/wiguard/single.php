 ...
 */
//Check CPT
if( class_exists( "WiguardCPT" ) ){
	if( is_singular( array( 'wiguard-portfolio', 'wiguard-team', 'wiguard-testimonial', 'wiguard-event', 'wiguard-services' ) ) ){
...
$full_width_class = '';
// Post View Count
$aps->wiguard_set_post_view_count( get_the_ID() );
?>
<div class="wiguard-content <?php echo esc_attr( 'wiguard-' . $template ); ?>">
	...
	<?php 
		if( $aps->wiguard_check_meta_value( 'wiguard_post_featured_slider', $template.'-featured-slider' ) ){
			$ahe->wiguard_featured_slider( $template );
		}
	?>
	
	<?php
		if( $aps->wiguard_check_meta_value( 'wiguard_post_full_wrap', 'single-post-full-wrap' ) ) : 
	?>
			<div class="post-full-thumb-wrap single-post-template">
				<?php
					if ( has_post_format( 'gallery' ) ) :
						$aps->wiguard_gallery_format();
				?>
				<?php elseif ( has_post_format( 'video' ) ) :
					...
					$video_modal = $aps->wiguard_check_meta_value( 'wiguard_post_video_modal', $template.'-video-format' );
					$video_atts = array(
							...
							<div class="post-overlay-items<?php echo ( has_post_format( 'image' ) ? ' thumb-exists' : '' ); ?>"> 
								<?php
									...
									if( $post_oitems_opt == '' || $post_oitems_opt == 'theme-default' ){
										$post_elements = WiguardThemeOpt::wiguard_static_theme_mod( 'single-post-overlay-items' );		
									...
									$aps->wiguard_post_overlay_items( $post_elements );
								?>
							</div>
						</div>
					</div>
				...
				<div class="<?php echo esc_attr( $template_class['content_class'] ); ?>">
					...
					<div id="primary" class="content-area">
						...
						<?php
							...
							$post_elements = array();
							$post_oitems_opt = get_post_meta( get_the_ID(), 'wiguard_post_page_items_opt', true );
							if( $post_oitems_opt == '' || $post_oitems_opt == 'theme-default' ){
								$post_elements = WiguardThemeOpt::wiguard_static_theme_mod( 'single-post-page-items' );	
								$post_elements = isset( $post_elements['Enabled'] ) ? $post_elements['Enabled'] : '';		
							}else{
								$overlay_post_items = get_post_meta( get_the_ID(), 'wiguard_post_page_items', true );
								$post_elements = json_decode( stripslashes( $overlay_post_items ), true );
								$post_elements = isset( $post_elements['Enabled'] ) ? $post_elements['Enabled'] : array();
							}
						...
						<div class="post-navigation-wrapper clearfix">
							<nav class="navigation post-navigation">
								<div class="nav-links custom-post-nav">
									...
									if (!empty( $prev_post )): ?>
										<?php
											$prev_title = $prev_post->post_title ? $prev_post->post_title : esc_html__( "Previous Post" , "wiguard" );
										?>
										<div class="prev-nav-link">
											<i class="ti-arrow-left"></i><span class="post-nav-text"><?php esc_html_e( "Prev" , "wiguard" ); ?></span>
											<a href="<?php echo esc_url( get_permalink( $prev_post->ID ) ); ?>" class="prev"><h4><?php echo esc_html( $prev_title ); ?></h4></a>
										</div>
									<?php endif; ?>
									...
									if (!empty( $next_post )): ?>
										<?php
											$next_title = $next_post->post_title ? $next_post->post_title : esc_html__( "Next Post" , "wiguard" );
										?>
										<div class="next-nav-link">
											<span class="post-nav-text"><?php esc_html_e( "Next" , "wiguard" ); ?></span><i class="ti-arrow-right"></i>
											<a href="<?php echo esc_url( get_permalink( $next_post->ID ) ); ?>" class="next"><h4><?php echo esc_html( $next_title ); ?></h4></a>
										</div>
									<?php endif; ?>
						...
						<?php 
							$comment_type = WiguardThemeOpt::wiguard_static_theme_mod( 'comments-type' );
							if( $comment_type == 'wp' ){
